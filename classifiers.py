import math

class cClassifier():
    def __init__(self):
        self.classifiers = [matchingRotation, areaRatio, betweenHeightRatio] #set the function names of classifiers to use here
        self.SIGMAS = [10, 1, 100] #Constants of where the sigmas start
    def classify(self, s1, s2, DEBUG): #returns true if it matches all of the classifiers, false if it fails any of them
        passFail = []
        if not DEBUG:
            for k, v in enumerate(self.classifiers):
                if not v(s1, s2, self.SIGMAS[k], False): #call the respective function
                    passFail.append(False)
                    return False
                    #print(passFail)
                passFail.append(True)
            return True
        else:
            values = []
            for k, v in enumerate(self.classifiers):
                values.append(v(s1, s2, self.SIGMAS[k], True))
            return values

def distance(x1, y1, x2, y2):
    return math.sqrt((x2 - x1)**2 + (y2 - y1)**2)

def areaRatio(s1, s2, sigma, getVal):
    area1 = s1[1][0]*s1[1][1]
    area2 = s2[1][0]*s2[1][1]
    areaRatio = area1/area2
    areaSigma = sigma
    if not getVal:
        return areaRatio < 1.5 + areaSigma and areaRatio > 1.5 - areaSigma
    else:
        return areaRatio
    
def centersToTopRatio(s1, s2, sigma, getVal):
    heightSigma = sigma
    centerDistance = distance(s1[0][0], s1[0][1], s2[0][0], s2[0][1])
    ratioToTop = centerDistance/s1[1][1]
    ratioToBottom = centerDistance/s2[1][1]
    if not getVal:
        return ratioToTop < 1.25 + heightSigma and ratioToTop > 1.25 - heightSigma
    else:
        return ratioToTop

def matchingRotation(s1, s2, sigma, getVal):
    angleSigma = sigma
    if not getVal:
        return s1[2] < s2[2] + angleSigma and s1[2] > s2[2] - angleSigma
    else:
        return s1[2]/(s2[2] + .001)

def betweenHeightRatio(s1, s2, sigma, getVal):
    betweenHeightSigma = sigma
    centerDistance = distance(s1[0][0], s1[0][1], s2[0][0], s2[0][1])
    betweenHeightRatio = centerDistance - (0.5*(s2[1][1]+s1[1][1]))/s1[1][1]
    maxRatio = 50
    minRatio = 20
    if not getVal:
        return betweenHeightRatio < maxRatio and betweenHeightRatio > minRatio
    else:
        return betweenHeightRatio
