...
class cClassifier():
    ...
    def classify(self, s1, s2, debug, target): #returns true if it matches all of the classifiers, false if it fails any of them
        ...
        for k, v in enumerate(classifiers):
            if debug: result, value = v(s1, s2, SIGMAS[k], debug) #call the respective function
            else: result = v(s1, s2, SIGMAS[k], debug)
            if debug:
                print(str(v) + ' returned ' + str(result))
                print('with a value of ' + str(value))
            if not result:
                passFail.append(False)
                if debug: print('Not a ' + str(target))
                return False
                #print(passFail)
            passFail.append(True)
        if debug: print('Is a ' + str(target))
        return True
        
    def getValues(self):
        return(passed, values)

def matchingRotation(s1, s2, sigma, debug):
    angleSigma = sigma
...
def goalAreaRatio(s1, s2, sigma, debug):
    area1 = s1[1][0]*s1[1][1]
    area2 = s2[1][0]*s2[1][1]
...
def goalBetweenHeightRatio(s1, s2, sigma, debug):
...
def minAreaDiff(s1, s2, sigma, debug):
    area1 = s1[1][0]*s1[1][1]
    area2 = s2[1][0]*s2[1][1]
    if debug: print 'Area diff: ' + str(area1 - area2)
    result = area1 - area2 < sigma and area1 - area2 > 0 - sigma
    if debug: return(result, area1 - area2)
    else: return result
    
def gearAreaRatio(s1, s2, sigma, debug):
    area1 = s1[1][0]*s1[1][1]
    area2 = s2[1][0]*s2[1][1]
    areaRatio = area1/area2
    areaSigma = sigma
...
def gearBetweenHeightRatio(s1, s2, sigma, debug):
    centerDistance = distance(s1[0][0], s1[0][1], s2[0][0], s2[0][1])
...
def centersToTopRatio(s1, s2, sigma, debug):
    heightSigma = sigma
    centerDistance = distance(s1[0][0], s1[0][1], s2[0][0], s2[0][1])
